...
- ✅ **Evidence log** — every novelty claim tied to query/source, date, result, conclusion, and confidence
- ✅ **Evaluation white papers** — each idea scored across 6 dimensions, with one-sentence reasons you can still understand later
- ✅ **Decision dashboard** — 索引.md shows every idea, its status, and its score at a glance
- ✅ **Consistency validator** — script checks that 索引.md, pending pool, and status folders agree
...
4️⃣  Evaluate: "Evaluate: [idea name]"
    ...
    → records each novelty check in evidence-log.md
...
> Every score gets one sentence explaining why. That sentence is the memory hook for future you.

### Consistency Check

Run this after moving ideas between statuses or editing `索引.md`:
...
It checks:
- ideas marked `待评估` are present in `01-灵感收集/待评估点子.md`
- ideas marked `评估中`, `进行中`, or `已归档` have matching folders
- folders in `02-评估中/`, `03-进行中/`, and `04-已归档/` are registered in `索引.md`
...
skill/
└── scripts/
    └── validate_idea_index.py # Lifecycle consistency validator
```
...
- ✅ **证据日志** — 每条新颖性判断都绑定查询词/来源、日期、结果、结论和置信度
- ✅ **评估白皮书** — 每个点子按 6 维度评分，并给每个分数留一句话理由
- ✅ **决策面板** — 索引.md 一眼看清所有点子及其状态和分数
- ✅ **一致性检查脚本** — 检查索引、待评估池和状态文件夹是否对齐
